...
$GLOBALS['TL_LANG']['tl_form_field']['autoIdAdditionalSqlWhere']  = array('Zusätzliche SQL WHERE Anweisung', 'Geben Sie zusätzliche SQL WHERE Anweisungen an. Die Verwendung dieser Option ist nur bei sehr guter SQL Kenntnis empfohlen.<br/>In einigen Systemen existieren bereits Ids für vorhandene Datensätze, die beibehalten werden müssen. Um nun z.B. sicher zu stellen, dass nur Zahlwerte zur Bestimmung der nächsten automatischen Id verwendet werden (also keine Werte wie etwa <i>Kunde 4711</i>) können diese mittels <code>{{autoid}} REGEXP \'^[0-9]\'</code> ausgeschlossen werden.<br/>Eine Kombination mehrerer Anweisungen ist mittels <code>AND</code> und <code>OR</code> möglich. Der Platzhalter <code>{{autoid}}</code> wird mit dem Feldwert Spaltennamen der automatischen Id ersetzt.');
